...
find . -name "holder.stl" | sort | while read -r file; do
    # Extract size from directory name (e.g., ./100ml/holder.stl -> 100ml)
    dir=$(dirname "$file")
    size=$(basename "$dir")
    ...
    echo "Exported to ${size}_holder.bgcode"
done
