$fn = 50;

difference() {
    union() {
        translate([2.2, 0, 0]) {
            cube([2.2, 7.2683, 0.22]);
        };
        union() {
            translate([0, 6.3598, 0]) {
                cube([6.6, 0.9085, 0.22]);
            };
            translate([2.2, 7.2683, 0]) {
                rotate([180, 270, 0]) {
                    linear_extrude(height=0.22) {
                        polygon([[0, 0], [2.8578838324886, 0], [0, 1.65]]);
                    };
                };
            };
            translate([4.18, 7.2683, 0]) {
                rotate([180, 270, 0]) {
                    linear_extrude(height=0.22) {
                        polygon([[0, 0], [2.8578838324886, 0], [0, 1.65]]);
                    };
                };
            };
        };
        difference() {
            union() {
                cube([6.6, 0.9085, 0.22]);
                translate([1.65, 0, 0]) {
                    cube([3.3, 0.22, 5.2064]);
                };
                translate([1.65, 0, 5.2064]) {
                    rotate([-30, 0, 0]) {
                        cube([3.3, 0.22, 1.299]);
                    };
                };
            };
            hull() {
                translate([3.3, 2.4603891360153, 5.6354]) {
                    rotate([60, 0, 0]) {
                        cylinder(h=7, r2=1.505, r1=1.505);
                    };
                };
                translate([3.3, 2.4603891360153, 7.1354]) {
                    rotate([60, 0, 0]) {
                        cylinder(h=7, r2=1.806, r1=1.806);
                    };
                };
            };
        };
        translate([3.3, 0.22, 0.22]) {
            rotate([0, 270, 0]) {
                linear_extrude(height=0.22) {
                    polygon([[0, 0], [4.7614, 0], [0, 4.4723595190148]]);
                };
            };
        };
    };
    translate([2.2, 0.4543, -0.1]) {
        rotate([0, 0, 180]) {
            linear_extrude(height=0.42) {
                text(valign="center", text="44d1d94", halign="left", size=0.396);
            };
        };
    };
    translate([6.38, 0.4543, -0.1]) {
        rotate([0, 0, 180]) {
            linear_extrude(height=0.42) {
                text(valign="center", text="G100O1", halign="left", size=0.396);
            };
        };
    };
};

translate([3.3, 7.2625, 3.0829]) {
    rotate([60, 0, 0]) {
        union() {
            translate([0, 0, 1.1]) {
                hull() {
                    translate([0, 0, 1.1]) {
                        cylinder(h=5.1, r2=1.5, r1=2.2);
                    };
                    rotate_extrude(convexity=10) {
                        translate([2.2, 0, 0]) {
                            circle(r=1.1);
                        };
                    };
                };
            };
            translate([0, 0, 7.3]) {
                union() {
                    cylinder(h=3.5, r2=1.5, r1=1.5);
                    translate([0, 0, 3.2]) {
                        rotate_extrude(convexity=10) {
                            translate([1.5, 0, 0]) {
                                circle(r=0.3);
                            };
                        };
                    };
                };
            };
        };
    };
};
