...
function build_base_rest_left(dims, thickness, total_length, neck_rest_base_length, base_width, skel, dt)
    ...
    foot_w = thickness * 3 / 2
    foot_l = b
    foot = anchored_cube({x = foot_w, y = foot_l, z = thickness})
    foot = cad.modify.translate(foot, {base_width - thickness / 2, total_length - foot_l, 0})

    rail = dovetail.build_rail_y(foot_l, dt.w_base, dt.w_neck, dt.h)
    rail = cad.modify.translate(rail, {base_width + thickness / 4, total_length - foot_l, 0})
...
function build_base_rest_right(dims, thickness, total_length, neck_rest_base_length, base_width, skel, dt)
    ...
    foot_l = b
    foot = anchored_cube({x = foot_w, y = foot_l, z = thickness})
    foot = cad.modify.translate(foot, {base_width * 2 - 3 * thickness / 2, total_length - foot_l, 0})

    rail = dovetail.build_rail_y(foot_l, dt.w_base, dt.w_neck, dt.h)
    rail = cad.modify.translate(rail, {base_width * 2 - 3 * thickness / 4, total_length - foot_l, 0})
